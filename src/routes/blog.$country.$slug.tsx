...
export const Route = createFileRoute("/blog/$country/$slug")({
  loader: async ({ params }) => {
    try {
  ...
  head: ({ loaderData, params }) => {
    ...
    const url = `https://www.7wingsimmigration.com/blog/${params.country}/${params.slug}`;
...
function BlogPostPage() {
  ...
  const hasHtml = !!post.contentHtml && post.contentHtml.trim().length > 0;

  // Inject ids into <h2> headings of the rich content so the TOC anchors work.
  const processedHtml = useMemo(() => {
    if (!hasHtml) return "";
    return post.contentHtml!.replace(/<h2([^>]*)>([\s\S]*?)<\/h2>/gi, (_m, attrs, inner) => {
      const id = slugify(String(inner).replace(/<[^>]+>/g, "").trim());
      return `<h2 id="${id}" class="scroll-mt-28"${attrs}>${inner}</h2>`;
    });
  }, [hasHtml, post.contentHtml]);
  ...
  const toc = useMemo(() => {
    if (hasHtml) {
      ...
      while ((m = re.exec(post.contentHtml!))) {
        const label = m[1].replace(/<[^>]+>/g, "").trim();
        if (label) out.push({ id: slugify(label), label });
      }
      return out;
    }
    return post.sections.map((s) => ({ id: slugify(s.heading), label: s.heading }));
  }, [hasHtml, post]);
        ...
        <div className="mx-auto grid max-w-[1200px] gap-10 px-6 lg:grid-cols-[minmax(0,1fr)_320px]">
          ...
          <div className="min-w-0">
            ...
            {hasHtml ? (
              ...
              <>
                <Reveal>
                  <p className="font-display text-lg leading-[1.5] text-navy-deep md:text-xl">{post.intro}</p>
                </Reveal>
            ...
            {/* CTA */}
            <Reveal>
              <div className="mt-10 flex flex-col gap-3 rounded-3xl border border-gold/30 bg-white p-6 sm:flex-row sm:items-center sm:justify-between md:p-8">
                <div>
                  <h3 className="font-display text-xl font-bold text-navy-deep">Ready to take the next step?</h3>
                  <p className="mt-1 text-sm text-muted-foreground">Talk to a senior 7 Wings consultant today.</p>
                </div>
            ...
            <div className="sticky top-28 space-y-6">
              ...
              <div>
                ...
                <div className="mt-3 flex gap-2">
                  <a href={`https://twitter.com/intent/tweet?text=${shareText}&url=${enc}`} target="_blank" rel="noopener noreferrer" aria-label="Share on Twitter" className="grid h-9 w-9 place-items-center rounded-full border border-black/10 bg-white text-navy-deep transition-all hover:-translate-y-0.5 hover:border-gold hover:text-gold-deep">
                    <Twitter className="h-4 w-4" />
                  </a>
                  <a href={`https://www.linkedin.com/sharing/share-offsite/?url=${enc}`} target="_blank" rel="noopener noreferrer" aria-label="Share on LinkedIn" className="grid h-9 w-9 place-items-center rounded-full border border-black/10 bg-white text-navy-deep transition-all hover:-translate-y-0.5 hover:border-gold hover:text-gold-deep">
                    <Linkedin className="h-4 w-4" />
                  </a>
                  <a href={`https://www.facebook.com/sharer/sharer.php?u=${enc}`} target="_blank" rel="noopener noreferrer" aria-label="Share on Facebook" className="grid h-9 w-9 place-items-center rounded-full border border-black/10 bg-white text-navy-deep transition-all hover:-translate-y-0.5 hover:border-gold hover:text-gold-deep">
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {related.map((r) => (
                <Link
                  ...
                  params={{ country: slugify(r.category), slug: r.slug }}
